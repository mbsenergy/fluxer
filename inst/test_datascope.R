...
dt_inst_items = datascope_instrument_items(
  instrument_list_id = dt_inst_list[Name == 'Forward_commodities'][1]$ListId,
...
dt_report_schedules = datascope_report_template_schedules(
  report_template_id = dt_report_templates[
    Name == 'eod_prices_volumes'
  ]$ReportTemplateId,
  auth_token = auth_token
)
...
dt_report_lastxtraction = datascope_report_lastextraction(
  schedule_id = dt_report_schedules$ScheduleId[1],
  auth_token = auth_token
)


## GET FILE ID FROM REPORTS LAST EXTRACTION ----------------------------------
dt_report_lastxtraction_file = datascope_report_extraction_files(
  report_extraction_id = dt_report_lastxtraction$ReportExtractionId[1],
  auth_token = auth_token
)


## GET FILE FROM REPORTS LAST EXTRACTION ----------------------------------
datascope_download_extracted_file(
  extracted_file_id = dt_report_lastxtraction_file$ExtractedFileId[1],
  auth_token = auth_token,
  filename = 'raw.csv'
)
...
eod_prices = datascope_report_template_schedules(
  report_template_id = dt_report_templates[
    Name == 'eod prices'
  ]$ReportTemplateId,
  auth_token = auth_token
)
eod_prices_volumes = datascope_report_template_schedules(
  report_template_id = dt_report_templates[
    Name == 'eod_prices_volumes'
  ]$ReportTemplateId,
  auth_token = auth_token
...
datascope_download_extracted_file(
  extracted_file_id = dt_report_lastxtraction_file$ExtractedFileId[1],
  auth_token = auth_token,
  filename = dt_report_lastxtraction_file$ExtractedFileName
)

dt_report_lastxtraction = datascope_report_lastextraction(
  schedule_id = dt_report_schedules$ScheduleId[2],
  auth_token = auth_token
)
dt_report_lastxtraction_file = datascope_report_extraction_files(
  report_extraction_id = dt_report_lastxtraction$ReportExtractionId[1],
